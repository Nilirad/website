...
Last year, I started participating to a [Bevy] working group
...
The development of this project happened out of the Bevy repository,
so synchronizing to the fast-paced development of the engine
...
it sends a `repository_dispatch` event to the subscribed GitHub repositories.
...
However, as all engineering projects,
...
Different from any type of programming I did in the past,
backend development is I/O-heavy,
meaning a server can have thousands of connections at each time,
and satisfying each request takes a significant amount of time,
most of which is spent not at processing data,
but at transferring it.
The traditional synchronous paradigm of fetching or sending data,
waiting for the operation to complete,
and then resume the control flow
...
_yielding_ the control flow of the native thread to the asyncrhonous runtime
([Tokio] in this case),
...
Tasks are an abstraction over both green and native threads,
where green threads are partitioned over a smaller number of native threads
to leverage multiple CPU cores and solving many OS scheduling idiosyncrasies.
CommitBridge spawns three asynchronous tasks
...
translating updates to `respository_dispatch` events for subscribers.
...
how do I notify the server for changes in a repository?
Two possibilities came up to my mind:
or the upstream repository workflow branch sends a wake up signal to the server
...
After that,
I simply used an MPSC channel
to send the updates discovered by the polling engine to the trigger engine.

### The trigger engine

The trigger engine woke up
each time it received a message from the MPSC channel.
...
When it comes to send the `repository_dispatch` event,
...
To solve this issue,
I added a new database table to use it as a trigger queue.
This way, if the process crashes, the job remains in the queue.
I also set the ability to attempt multiple times
using an exponential backoff strategy to mitigate the effect of network errors.
...
In simple terms, I attempt queueing the job,
...
That discourages people from continuing developing it.
On the contrary,
focusing too much over code quality
...
Technical debt also cumulates interest:
the more you wait to fix it, the more it is difficult to pay it out,
and if it involves public-facing APIs
you're forced to introduce a breaking change to solve it.

Can we pay out all technical debt, or avoid it completely?
...
Having a CI pipeline that can lint, test and build your code,
and more recently,
...
Then, periodically pay up those bills.
...
I already have a couple more versions in program.
...
and rate limit outgoing requests toward GitHub.
...
My biggest error didn't affect much the diff size, but was a vision overreach:
...
At the end I had to use AI to make an incremental but extensive refactor
...
but I procrastinated its implementation
...
This would have allowed cleaner business logic on day-one and less safety bugs.

At the end I learned that prototyping shouldn't be an excuse
...
I feel quite satisfied about how the project went,
...
If you are interested in trying it out, you can check out [my repository],
just keep in mind that the project still has some rough edges.

<!-- LINKS -->
[my repository]: https://github.com/Nilirad/commit-bridge
